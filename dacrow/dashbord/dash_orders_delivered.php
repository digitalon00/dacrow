<?php
    ...
    if(isset($_POST['accept'])){
        ...
        $order_id = filter_var($order_id,FILTER_SANITIZE_STRING);
            ...
            $update_order = $conn->prepare("UPDATE `orders` SET status='accept' WHERE id=?");
            $update_order -> execute([$order_id]);
        }
?>
...
<body>
        <section class="dashboard">
            ...
            <div class="dashboard-container">
                ...
                <div class="dashroot">
                    <p>My Dashboard / <span class="dash-root">Manage Orders / Delivered</span></p>
                </div>
                <div class="dash-box">
                    <div class="orders_list">
                        ...
                        <div class="orders_table">
                            <table>
                                ...
                                <tbody>
                                    <?php
                                    $select_order=$conn->prepare("SELECT * FROM `orders` WHERE status='delivered' ORDER BY user_id DESC");
                                    ...
                                    if ($select_order->rowCount()>0) {
                                        while ($fetch_order = $select_order -> fetch(PDO::FETCH_ASSOC)) {
                                            ...
                                            <td><form action="" method="post">
                                                ...
                                                <a href="dash_order_details?pid=<?php echo $fetch_order['id'] ?>">Details</a>
                                                <button type="submit" name="accept" class="accept_btn" onclick="return confirm('back to accept ??')">back to accept</button>
                                                <button type="submit" name="done" class="done_btn" onclick="return confirm('done ??')">done</button>
