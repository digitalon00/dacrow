<?php
    ...
    if(isset($_GET['pid'])){
        ...
        $update_product=$conn->prepare("UPDATE `admins` SET username=?,email=?,password=?,role=? WHERE id='$pid'");
        $update_product->execute([$username,$email,$password,$role]);
...
<body>
        <section class="dashboard">
            ...
            <div class="dashboard-container">
                ...
                <div class="dashroot">
                    <p>My Dashboard / <span class="dash-root">Admins / Manage / Update Admin </span></p>
                </div>
                <div class="dash-box">
                    ...
                    <div class="product_update">
                            ...
                            if(isset($_GET['pid'])){
                            $pid = $_GET['pid'];
                            $select_admins_table = $conn  -> prepare("SELECT * FROM `admins` WHERE id='$pid'");
                            $select_admins_table ->execute();
                            if ($select_admins_table->rowCount()>0) {
                                while($fetch_admins_table = $select_admins_table->fetch(PDO::FETCH_ASSOC)){
                            ?>
                            <form action="" method="post">
                                <h3>ID : <span><?=$fetch_admins_table["id"]?></span></h3>
                                <h3>User : <input type="text" value="<?=$fetch_admins_table["username"]?>" name="username"></h3>
                            <h3>Email : <input type="email" value="<?=$fetch_admins_table["email"]?>" name="email"></h3>
                            <h3>Pass : <input type="text" value="<?=$fetch_admins_table["password"]?>" name="password"></h3>
                            <h3>Rôle : <input type="text" value="<?=$fetch_admins_table["role"]?>" name="role"></h3>
                                <input type="hidden" name="product_id" value="<?=$fetch_admins_table['id'];?>">
                                <button type="submit" name="update" onclick="return confirm('update this admin ??')">Update</button>
                            </form>
                                <?php
                            };
                        }
